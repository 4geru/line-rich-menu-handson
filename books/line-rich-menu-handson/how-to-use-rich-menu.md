...
## リッチメニューでできること
リッチメニューでは、 [アクションオブジェクト](https://developers.line.biz/ja/reference/messaging-api/#action-objects) を利用できます。
現在は6種類のアクションが使えます。今回のハンズオンでは、 `URIアクション` と `リッチメニュー切り替えアクション` を利用します。
